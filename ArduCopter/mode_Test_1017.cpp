...
Vector3p Test_target_position_cm;

bool guided_pos_terrain_alt_t;                // true if guided_pos_target_cm.z is an alt above terrain
...
bool ModeTest_1017::init(bool ignore_checks)
{
    ...
    //my init
    path_num=0;
    for_static_target();    //生成航线
    wp_control_start();    //开始位置控制
    //pva_control_start();
...
void ModeTest_1017::run()
{
    ...
    if (path_num!=3 && wp_nav->reached_wp_destination()) {  // 到达某个端点
        path_num++;
        wp_nav->set_wp_destination(path[path_num], false);  // 将下一个航点位置设置为导航控制模块的目标位置
        gcs().send_text(MAV_SEVERITY_CRITICAL, "achieve");
        
        //wp_nav->set_speed_xy(2000.0f);//cms
        //wp_nav->set_speed_up(200.0f);
        //wp_nav->set_speed_down(200.0f);
    }
    else if ((path_num == 3) && wp_nav->reached_wp_destination()) {  // 五角星航线运行完成，自动进入Loiter模式
        gcs().send_text(MAV_SEVERITY_CRITICAL, "mission finished, now go into loiter mode");
        copter.set_mode(Mode::Number::LOITER, ModeReason::MISSION_END);  // 切换到loiter模式
    }
    //guided_vel_target_cms = Vector3f(500.0f, 500.0f, 500.0f);

    wp_nav->set_speed_xy(2000.0f);//cms
    //wp_nav->set_speed_up(50.0f);
    //wp_nav->set_speed_down(50.0f);
    wp_control_run();  // wp控制器


 }

bool ModeTest_1017::allows_arming(AP_Arming::Method method) const
{
...
bool ModeTest_1017::allows_weathervaning() const
{
...
bool ModeTest_1017::do_user_takeoff_start(float takeoff_alt_cm)
{
...
void ModeTest_1017::wp_control_start()
{
...
void ModeTest_1017::wp_control_run()
{
...
void ModeTest_1017::pva_control_start()
{
    ...
    guided_pos_terrain_alt_t = false;
}

// initialise guided mode's position controller
void ModeTest_1017::pos_control_start()
{
    // set to position control mode
...
void ModeTest_1017::accel_control_start()
{
...
void ModeTest_1017::velaccel_control_start()
{
    // set guided_mode to velocity controller
...
void ModeTest_1017::posvelaccel_control_start()
{
    // set guided_mode to velocity controller
...
bool ModeTest_1017::is_taking_off() const
{
    return guided_mode == SubMode::TakeOff && !takeoff_complete;
}

bool ModeTest_1017::set_speed_xy(float speed_xy_cms)
{
    // initialise horizontal speed, acceleration
...
bool ModeTest_1017::set_speed_up(float speed_up_cms)
{
    // initialize vertical speeds and acceleration
...
bool ModeTest_1017::set_speed_down(float speed_down_cms)
{
    // initialize vertical speeds and acceleration
...
void ModeTest_1017::angle_control_start()
{
    // set guided_mode to velocity controller
...
bool ModeTest_1017::set_destination(const Vector3f& destination, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw, bool terrain_alt)
{
#if AP_FENCE_ENABLED
    ...
    if (use_wpnav_for_position_control()) {
        ...
        // log target
        //copter.Log_Write_Guided_Position_Target(guided_mode, destination, terrain_alt, Vector3f(), Vector3f());       //有问题
    ...
    guided_pos_terrain_alt_t = terrain_alt;
    guided_vel_target_cms.zero();
    guided_accel_target_cmss.zero();
    update_time_ms = millis();

    // log target
    //copter.Log_Write_Guided_Position_Target(guided_mode, guided_pos_target_cm.tofloat(), guided_pos_terrain_alt_t, guided_vel_target_cms, guided_accel_target_cmss);
    //有问题
    send_notification = true;
...
bool ModeTest_1017::get_wp(Location& destination) const
{
    switch (guided_mode) {
    case SubMode::WP:
        return wp_nav->get_oa_wp_destination(destination);
    case SubMode::Pos:
        destination = Location(guided_pos_target_cm.tofloat(), guided_pos_terrain_alt_t ? Location::AltFrame::ABOVE_TERRAIN : Location::AltFrame::ABOVE_ORIGIN);
        return true;
    default:
        return false;
    }
...
bool ModeTest_1017::set_destination(const Location& dest_loc, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw)
{
#if AP_FENCE_ENABLED
    ...
    // log target
    //copter.Log_Write_Guided_Position_Target(guided_mode, Vector3f(dest_loc.lat, dest_loc.lng, dest_loc.alt), guided_pos_terrain_alt_t, guided_vel_target_cms, guided_accel_target_cmss);
...
void ModeTest_1017::set_accel(const Vector3f& acceleration, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw, bool log_request)
{
...
void ModeTest_1017::set_velocity(const Vector3f& velocity, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw, bool log_request)
{
    set_velaccel(velocity, Vector3f(), use_yaw, yaw_cd, use_yaw_rate, yaw_rate_cds, relative_yaw, log_request);
}

// set_velaccel - sets guided mode's target velocity and acceleration
void ModeTest_1017::set_velaccel(const Vector3f& velocity, const Vector3f& acceleration, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw, bool log_request)
{
    // check we are in velocity control mode
...
bool ModeTest_1017::set_destination_posvel(const Vector3f& destination, const Vector3f& velocity, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw)
{
...
bool ModeTest_1017::set_destination_posvelaccel(const Vector3f& destination, const Vector3f& velocity, const Vector3f& acceleration, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw)
{
#if AP_FENCE_ENABLED
    // reject destination if outside the fence
...
bool ModeTest_1017::set_attitude_target_provides_thrust() const
{
...
bool ModeTest_1017::stabilizing_pos_xy() const
{
...
bool ModeTest_1017::stabilizing_vel_xy() const
{
...
bool ModeTest_1017::use_wpnav_for_position_control() const
{
...
void ModeTest_1017::set_angle(const Quaternion &attitude_quat, const Vector3f &ang_vel, float climb_rate_cms_or_thrust, bool use_thrust)
{
    // check we are in velocity control mode
...
void ModeTest_1017::takeoff_run()
{
    auto_takeoff_run();
...
void ModeTest_1017::pos_control_run()
{
    // if not armed set throttle to zero and exit immediately
    if (is_disarmed_or_landed()) {
        // do not spool down tradheli when on the ground with motor interlock enabled
    ...
    if (guided_pos_terrain_alt_t && !wp_nav->get_terrain_offset(terr_offset)) {
        // failure to set destination can only be because of missing terrain data
...
// called from guided_run
void ModeTest_1017::accel_control_run()
{
    // if not armed set throttle to zero and exit immediately
    if (is_disarmed_or_landed()) {
        // do not spool down tradheli when on the ground with motor interlock enabled
...
// called from guided_run
void ModeTest_1017::velaccel_control_run()
{
    // if not armed set throttle to zero and exit immediately
    if (is_disarmed_or_landed()) {
        // do not spool down tradheli when on the ground with motor interlock enabled
...
// called from guided_run
void ModeTest_1017::pause_control_run()
{
    // if not armed set throttle to zero and exit immediately
    if (is_disarmed_or_landed()) {
        // do not spool down tradheli when on the ground with motor interlock enabled
...
// called from guided_run
void ModeTest_1017::posvelaccel_control_run()
{
    // if not armed set throttle to zero and exit immediately
    if (is_disarmed_or_landed()) {
        // do not spool down tradheli when on the ground with motor interlock enabled
...
// called from guided_run
void ModeTest_1017::angle_control_run()
{
...
void ModeTest_1017::set_yaw_state(bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_angle)
{
...
bool ModeTest_1017::use_pilot_yaw(void) const
{
...
void ModeTest_1017::limit_clear()
{
    guided_limit_t.timeout_ms = 0;
    guided_limit_t.alt_min_cm = 0.0f;
    guided_limit_t.alt_max_cm = 0.0f;
    guided_limit_t.horiz_max_cm = 0.0f;
}

// limit_set - set guided timeout and movement limits
void ModeTest_1017::limit_set(uint32_t timeout_ms, float alt_min_cm, float alt_max_cm, float horiz_max_cm)
{
    guided_limit_t.timeout_ms = timeout_ms;
    guided_limit_t.alt_min_cm = alt_min_cm;
    guided_limit_t.alt_max_cm = alt_max_cm;
    guided_limit_t.horiz_max_cm = horiz_max_cm;
}

// limit_init_time_and_pos - initialise guided start time and position as reference for limit checking
//  only called from AUTO mode's auto_nav_guided_start function
void ModeTest_1017::limit_init_time_and_pos()
{
    // initialise start time
    guided_limit_t.start_time = AP_HAL::millis();

    // initialise start position from current position
    guided_limit_t.start_pos = inertial_nav.get_position_neu_cm();
}

// limit_check - returns true if guided mode has breached a limit
//  used when guided is invoked from the NAV_GUIDED_ENABLE mission command
bool ModeTest_1017::limit_check()
{
    // check if we have passed the timeout
    if ((guided_limit_t.timeout_ms > 0) && (millis() - guided_limit_t.start_time >= guided_limit_t.timeout_ms)) {
        return true;
    }
    ...
    if (!is_zero(guided_limit_t.alt_min_cm) && (curr_pos.z < guided_limit_t.alt_min_cm)) {
        return true;
    }

    // check if we have gone above max alt
    if (!is_zero(guided_limit_t.alt_max_cm) && (curr_pos.z > guided_limit_t.alt_max_cm)) {
        return true;
    }

    // check if we have gone beyond horizontal limit
    if (guided_limit_t.horiz_max_cm > 0.0f) {
        const float horiz_move = get_horizontal_distance_cm(guided_limit_t.start_pos.xy(), curr_pos.xy());
        if (horiz_move > guided_limit_t.horiz_max_cm) {
            return true;
        }
    }
...
const Vector3p &ModeTest_1017::get_target_pos() const
{
    return guided_pos_target_cm;
}

const Vector3f& ModeTest_1017::get_target_vel() const
{
    return guided_vel_target_cms;
}

const Vector3f& ModeTest_1017::get_target_accel() const
{
    return guided_accel_target_cmss;
}

uint32_t ModeTest_1017::wp_distance() const
{
    switch(guided_mode) {
    case SubMode::WP:
...
int32_t ModeTest_1017::wp_bearing() const
{
    switch(guided_mode) {
    case SubMode::WP:
...
float ModeTest_1017::crosstrack_error() const
{
    switch (guided_mode) {
    case SubMode::WP:
...
uint32_t ModeTest_1017::get_timeout_ms() const
{
    return MAX(copter.g2.guided_timeout, 0.1) * 1000;
}

// pause guide mode
bool ModeTest_1017::pause()
{
    _paused = true;
    return true;
}
...
/*void ModeTest_1017 :: for_static_target(){

    float test_yaw = 90.0f;
    float test_pitch = 10.0f;  
    float test_distance = 10000; // cm
    float target_x = test_distance * cosf(test_pitch) * cosf(test_yaw);
    float target_y = test_distance * cosf(test_pitch) * sinf(test_yaw);
    float target_z = test_distance * sinf(test_pitch);

    Vector3f target_position(target_x, target_y, target_z); //计算目标

    Vector3f target_velocity(500.0f, 500.0f, 500.0f); // 设速度？
    Vector3f target_acceleration(0.0f, 0.0f, 0.0f); // 根据需求设定

    //float desired_yaw_rate_cd = 15.0f;
    //auto_yaw.set_fixed_yaw(desired_yaw_cd, desired_yaw_rate_cd, 1, true);

    pos_control->set_pos_target_xy_cm(target_x,target_y);
    pos_control->set_pos_target_z_cm(target_z);


    //set_destination_posvelaccel(target_position, target_velocity, target_acceleration, true, test_yaw, true, desired_yaw_rate_cd, true);  
...
void ModeTest_1017::run()
{
    ...
    // call the correct auto controller
    if(ModeTest_1017::flag1==0){
        for_static_target();
        ModeTest_1017::flag1=1;
    }
...
void ModeTest_1017::for_static_target(){
    ...
    path[1] = path[0] + Vector3f(1.0f, 0, 0) * 50000;
    path[2] = path[1] + Vector3f(0 , 0, 1.0f) * 2000;
    //path[3] = path[2] + Vector3f(0 , 0, -1.0f) * 1000;
    //path[4] = path[3] + Vector3f(0 , 1.0f, 0) * 3000;
    //path[5] = path[4] + Vector3f(1.0f , 0, -1.0f) * 1000;
    path[3] = path[2] + Vector3f(cosf(radians(test_pitch)) * cosf(radians(test_yaw)), //x
                                 cosf(radians(test_pitch)) * sinf(radians(test_yaw)), //y
                                 -sinf(radians(test_pitch)))                 //z
